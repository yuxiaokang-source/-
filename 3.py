# -*- coding: utf-8 -*-
"""
Created on Sun Jun  7 19:31:16 2020
...
for i in range(0,2):
    temp_x=np.random.normal(loc=i,scale=1,size=(600,1000))
...
temp=model.predict([testData,test_label])
print(temp)

a=np.random.normal(loc=0.0,scale=2.0,size=(2,1000))
label_a=np.array([[0],[1]],dtype="int32")
temp_a=model.predict([a,label_a])#每个样本对应都由一个值
print(temp_a)
mmd_a=MMD_model([a,label_a])
print(mmd_a)

recons_a=reconstruction_model([a,label_a])
print(recons_a)
print("total_loss")
print(np.array(mmd_a)+labda*np.array(recons_a))
sess.close()

#问题所在，这个地方两个子模型相加的损失刚好时总模型的，为什么和之前的时不一样的
#现在有点奇怪